defmodule ErpExWeb.PurchaseOrderLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all purchase_orders", %{conn: conn, purchase_order: purchase_order} do
      {:ok, _index_live, html} = live(conn, ~p"/purchase_orders")

      assert html =~ "Listing Purchase orders"
      assert html =~ purchase_order.notes
    end
    ...
    test "updates purchase_order in listing", %{conn: conn, purchase_order: purchase_order} do
      ...
      assert index_live |> element("#purchase_orders-#{purchase_order.id} a", "Edit") |> render_click() =~
               "Edit Purchase order"
    ...
    test "deletes purchase_order in listing", %{conn: conn, purchase_order: purchase_order} do
      ...
      assert index_live |> element("#purchase_orders-#{purchase_order.id} a", "Delete") |> render_click()
